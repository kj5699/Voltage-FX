...
function formatReport(
  ...
  return [
    ...
    '',
    '  Worker-thread pipeline cost/second (extrapolated — pipelines run in pipelineWorker.ts):',
    ...
    `    Total   : ${totalCost.toFixed(0).padStart(4)}ms/s  (main thread cost ≈ 0 — only store.set + React render)`,
    '',
    '  Recommendation:',
    recommendation,
    '',
